...
class PlayerView extends React.Component {
  componentDidMount () {
    if (this.player.audioEl) { this.player.audioEl.volume = playerState().volume }
  }
  componentWillUnmount () {
    if (this.player.audioEl) { playerState('SET_VOLUME', this.player.audioEl.volume) }
  }
  render () {
    const { title, artist, src, onPlayNextTrack, onPlayPrevTrack, playerStateForm } = this.props
    const { shuffle, repeat, volume } = playerStateForm
    console.log(volume)
    return (
